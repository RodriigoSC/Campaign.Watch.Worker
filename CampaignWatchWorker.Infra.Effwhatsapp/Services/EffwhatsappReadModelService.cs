...
namespace CampaignWatchWorker.Infra.Effwhatsapp.Services
{
    public class EffwhatsappReadModelService : IEffwhatsappReadModelService
    {
        private readonly IEffwhatsappMongoFactory _factory;

        public EffwhatsappReadModelService(IEffwhatsappMongoFactory factory)
        {
            _factory = factory;
        }
        ...
        public async Task<IEnumerable<EffwhatsappReadModel>> GetTriggerEffwhatsapp(string dbName, string workflowId)
        {
            ...
            var aggregation = await triggerCollection.Aggregate<EffwhatsappReadModel>(pipeline).ToListAsync();
            return aggregation;
        }
    }
}
