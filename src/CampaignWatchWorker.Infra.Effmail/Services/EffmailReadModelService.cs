...
namespace CampaignWatchWorker.Infra.Effmail.Services
{
    public class EffmailReadModelService : IEffmailReadModelService
    {
        private readonly IEffmailMongoFactory _factory;

        public EffmailReadModelService(IEffmailMongoFactory factory)
        {
            _factory = factory;
        }
        ...
        public async Task<IEnumerable<EffmailReadModel>> GetTriggerEffmail(string workflowId)
        {
            ...
            var aggregation = await triggerCollection.Aggregate<EffmailReadModel>(pipeline).ToListAsync();
            return aggregation;
        }
    }
}
